...
import { 
  ...
  RefreshCw, AlertCircle, CheckCircle, Rocket, User
} from 'lucide-react'
...
export default function MemberRecommendationsTab() {
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      console.log('[MemberRecommendations] Response status:', response.status)
      
      if (!response.ok) {
        const errorText = await response.text()
        console.error('[MemberRecommendations] Error response:', errorText)
      ...
      console.log('[MemberRecommendations] Data received:', data)
      
      // Check if recommendations exist in the response
      if (data && data.recommendations) {
        setRecommendations(data.recommendations)
      } else {
        console.warn('[MemberRecommendations] No recommendations in response')
        setError('No recommendations data received')
      }
    } catch (err) {
      console.error('[MemberRecommendations] Error loading recommendations:', err)
      ...
      <div>
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-12 border border-white/10 text-center">
          ...
          <p className="text-white/60 mb-6 max-w-md mx-auto">
            Complete an assessment to receive personalized recommendations for courses and tools
          </p>
          <button
    ...
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-lg text-white/80">
            {recommendations.metadata.memberName && (
              <span className="font-medium">{recommendations.metadata.memberName}'s </span>
            )}
      ...
      {recommendations.insights.aiSummary && (
        <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 backdrop-blur-sm rounded-xl p-6 border border-white/20 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">Your Development Focus</h3>
        ...
        {recommendations.insights.topChallenges.length > 0 && (
          <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-medium text-white">Your Challenges</h4>
            </div>
