...
// Real Campfire Course/Tool catalog with metadata for matching
...
export async function GET(request: NextRequest) {
  try {
    ...
    const invitations = await prisma.invitation.findMany({
      ...
      take: 20 // Get recent completions
    });
    
    // Aggregate data from assessments
    ...
    invitations.forEach(invitation => {
      invitation.assessmentResults.forEach(result => {
        ...
        if (result.responses && typeof result.responses === 'object') {
          ...
          if (responses.priorities) {
            const prioritiesArray = Array.isArray(responses.priorities) 
              ? responses.priorities 
              : [responses.priorities];
            prioritiesArray.forEach((priority: string) => {
              if (priority) {
                const count = aggregatedData.focusAreas.get(priority) || 0;
                aggregatedData.focusAreas.set(priority, count + 1);
              }
            });
          }
        }
    ...
    let aiInsights = null;
    if (topChallenges.length > 0 && user) {
      // Check rate limit for OpenAI calls
      const rateLimitCheck = checkOpenAIRateLimit(user.id);
      
      if (!rateLimitCheck.allowed) {
        console.log(`[recommendations] Rate limit exceeded for user ${user.id}, retry after ${rateLimitCheck.retryAfter}s`);
        // Don't fail the whole request, just skip AI insights
        aiInsights = null;
      } else {
        try {
          const prompt = `Based on this team's assessment data:
          Top Challenges: ${topChallenges.join(', ')}
          Skills Gaps: ${topSkills.join(', ')}
          Support Needs: ${topNeeds.join(', ')}
          Focus Areas: ${topFocusAreas.join(', ')}
          
          Provide a brief (2-3 sentences) strategic recommendation for leadership development priorities.`;
    ...
    return NextResponse.json({
      recommendations: {
        ...
        metadata: {
          totalAssessments: invitations.length,
          lastUpdated: new Date().toISOString()
        }
      }
    });
