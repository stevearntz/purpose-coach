  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // User exists - send verification code immediately
        ...
        if (emailCodeFactor && 'emailAddressId' in emailCodeFactor) {
          ...
          // Fallback to password if email code not available
          const redirectParam = redirectUrl ? `&redirect_url=${encodeURIComponent(redirectUrl)}` : ''
          router.push(`/auth/manual?email=${encodeURIComponent(email)}${redirectParam}`)
        }
      } catch (err: any) {
        // Check if error is because user doesn't exist
        if (err.errors?.[0]?.code === 'form_identifier_not_found') {
          // User doesn't exist - show name collection form
          console.log('[Auth] User does not exist, showing name collection')
  ...
  const handleNameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUpErr.errors?.[0]?.code === 'form_identifier_exists') {
        // User actually exists, try sign-in flow
        setShowNameCollection(false)
        setFirstName('')
        setLastName('')
        handleEmailSubmit(e) // Try again as existing user
      } else {
            ...
            <form onSubmit={handleNameSubmit} className="space-y-4">
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Welcome!</strong> Let's set up your account for <strong>{email}</strong>
                </p>
              </div>
